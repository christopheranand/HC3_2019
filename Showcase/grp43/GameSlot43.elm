-- the only change you should need to make is to add these three lines (and change the group number)
...
-- this application was developed by group 43 on macoutreach.rocks. This code can
-- also be found under the user DeterminedDonkey in game slot 8.
...
myShapes model =
  case model.page of
      ...
      Game ->
          ...
          ,
          text ("Score: " ++ String.fromInt model.gameScore ++ "/10" )|> filled black
                     |> move (-20,-50)
          ,
          text ("Time: " ++ (String.fromInt <| round (model.time-model.startTime))) |> filled black
                     |> move (40,-50)
          ]
          ++
          dynamicGame model -- this is the matching game that is rendered on the game page.
...
title label = group [text label |> centered |> size 3 |> filled black ]  |> move (-85,60)

-- standard change page button function.
changePageButton goTo label dx dy= group [
...
selectGameTypeButton select label dx dy gt model = group [
...
selected gt model =
  case gt of
    Logic -> if (model.gameType == Logic) then grey else white
    SetTheory -> if (model.gameType == SetTheory) then grey else white
    Integers -> if (model.gameType == Integers) then grey else white
...
theoremNameCell theoremName dx dy = group [
...
emptyCell dx dy = rect 50 8 |> outlined (solid 1) (rgb 0 0 0) |> move (dx, dy)
...
theoremNameCell2 theoremName dx dy = group [
                                    text theoremName |> centered |> size 4 |> filled red|> move(0,-2.5)
                                    ] |> move (0, dy)
...
dynamicGame model =
  let
    lList = Dict.toList model.targetDict
  in
...
drawWord : Int -> (String, (List (List (Float,Float)) -- the colour and target region
                , (Float, Float))) -> Shape Msg
...
update msg model = case msg of
                     Tick t _ -> --{ time = t, page = model.page, gameType = model.gameType, state = model.state,  }
                       case model.state of
                          ...
                          ProclaimingVictory label tuple
                            -> { model | time = t
                                       ...
                                       , page = if model.gameScore == 9 -- is one less than 10
                                           then
                                             Results
                                           else
                                             Game
                     ...
                     GoToHome -> { model | page = Home }
                     GoToGame -> { model | page = Game,
                                           startTime = model.time,
                                           gameScore = 0,
                                           targetDict = case model.gameType of
                                                           Logic -> randomize logicList model.time
                                                           SetTheory -> randomize setList model.time
                                                           Integers -> randomize integerList model.time
                                                           }
                     Submit -> { model | page = Results }
                     GoToTheoremList -> { model | page = TheoremList }
                     GoToHowToPlay -> { model | page = HowToPlay }

                     SelectLogic -> { model | gameType = Logic }
                     SelectSetTheory -> { model | gameType = SetTheory }
                     SelectIntegers -> { model | gameType = Integers }
...
randomizeHelper time dictElem =
  let
    heights = (shuffleList (initialSeed (round time)) randomCellList)
  in
    case dictElem of
      (label,(polys,(x,i))) -> (label,(polys,(x,(Maybe.withDefault 100 (getAt (round i) heights)))))

-- initial values.
init = { time = 0, startTime = 0, totalTime = 0, page = Home, gameType = Logic, gameScore = 0, state = Waiting, targetDict = Dict.fromList logicList }
...
randomCellList = [40-2.5,32-2.5,24-2.5,16-2.5,8-2.5,0-2.5,-8-2.5,-16-2.5,-24-2.5,-32-2.5]

logicList : List ( String  -- the theorem
                 , (List (List (Float,Float)) -- the target region
                   , (Float, Float)) -- the theorems start location. (updated to the target when user makes a correct match)
                 )
logicList = [ ("p ≡ q ≡ q ≡ p", ([[(0,46), (25,36), (-25,36), (0,46)]], (50,0) ))
...
setList : List ( String  -- the theorem
                , (List (List (Float,Float)) -- the target region
                , (Float, Float)) -- the theorems start location. (updated to the target when user makes a correct match)
                 )
setList = [ ("R = S ⇒ R ⊆ S", ([[(0,46), (25,36), (-25,36), (0,46)]], (50,0) ))
...
integerList : List ( String  -- the theorem
                , (List (List (Float,Float)) -- the target region
                , (Float, Float)) -- the theorems start location. (updated to the target when user makes a correct match)
                 )
integerList = [ ("a > b ≡ b < a", ([[(0,46), (25,36), (-25,36), (0,46)]], (50,0) ))
